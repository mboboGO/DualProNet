...
class Model(nn.Module):
    def __init__(self, pretrained=True, args=None):
        ...
        self.zsr_decoder = TransformerDecoder(decoder_layer, num_decoder_layers, nn.LayerNorm(d_model),
                                          return_intermediate=True)
                                                                                
        self.zsr_pro = nn.Sequential(
            nn.Conv2d(feat_dim, n_att_emb, kernel_size=1, stride=1, padding=0,bias=False),
        ...
        self.zsr_aux = nn.Linear(n_att_emb, num_classes)
    ...
    def forward(self, x):
        ...
        print('aaa',tgt.size(),memory.size(),query_embed.size())
        aaaa
        hs = self.zsr_decoder(tgt, memory, pos=pos_emb, query_pos=query_embed)
        
        memory = memory.permute(1, 2, 0).view(bs, c, h, w)
        hsp = [h.permute(1, 0, 2) for h in hs]
        # out
        for h in hsp: 
            zsr_x = self.zsr_pro(h).squeeze().view(bs,-1)
...
def transformer(pretrained=True, loss_params=None, args=None):
