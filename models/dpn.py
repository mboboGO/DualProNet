...
class DPN(nn.Module):
    def __init__(self, pretrained=True, args=None):
        ...
        if self.args.n_dec==0:
            ...
            # att prt
            self.zsl_prt_emb = nn.Embedding(sf_size, vis_emb_dim)
            ...
            self.cls_prt_dec = _get_clones(PrtClsLayer(nc=num_classes, na=sf_size, dim=emb_dim), args.n_dec)
            # sem proj
            self.sem_proj = nn.Sequential(
